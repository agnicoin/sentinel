...
@pytest.fixture
def valid_agni_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'AabYBvoyjWwKSBZcezKfdPYzCuBAXEcFit'


@pytest.fixture
def invalid_agni_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'AabYBvoyjWwKSBZcezKfdPYzCuBAXEcFir'
...
def test_valid_agni_address():
    from agnilib import is_valid_agni_address

    main = valid_agni_address()
    test = valid_agni_address('testnet')

    assert is_valid_agni_address(main) is True
    assert is_valid_agni_address(main, 'mainnet') is True
    assert is_valid_agni_address(main, 'testnet') is False

    assert is_valid_agni_address(test) is False
    assert is_valid_agni_address(test, 'mainnet') is False
    assert is_valid_agni_address(test, 'testnet') is True


def test_invalid_agni_address():
    from agnilib import is_valid_agni_address

    main = invalid_agni_address()
    test = invalid_agni_address('testnet')

    assert is_valid_agni_address(main) is False
    assert is_valid_agni_address(main, 'mainnet') is False
    assert is_valid_agni_address(main, 'testnet') is False

    assert is_valid_agni_address(test) is False
    assert is_valid_agni_address(test, 'mainnet') is False
    assert is_valid_agni_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from agnilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % agnilib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(agnilib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(agnilib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(agnilib.blocks_to_seconds(16616)) == 2612035
