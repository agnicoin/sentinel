...
def has_agni_conf():
    ...
    valid_agni_conf = False

    # ensure agni_conf exists & readable
    #
    # if not, print a message stating that Agni Core must be installed and
    # configured, including JSONRPC access in agni.conf
    try:
        f = io.open(config.agni_conf)
        valid_agni_conf = True
    except IOError as e:
        print(e)

    return valid_agni_conf
...
def main():
    ...
    if not has_agni_conf():
        print("AgniCore must be installed and configured, including JSONRPC access in agni.conf")
        sys.exit(1)
