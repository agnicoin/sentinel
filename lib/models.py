...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, agnid):
        golist = agnid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_agnid(agnid, item)
    ...
    @classmethod
    def import_gobject_from_agnid(self, agnid, rec):
        import decimal
        import agnilib
        import inflection
        ...
        # shim/agnid conversion
        object_hex = agnilib.SHIM_deserialise_from_agnid(object_hex)
        objects = agnilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from agnid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from agnid, with every run
        ...
        # get/create, then sync payment amounts, etc. from agnid - Agnid is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(agnid)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from agnid! %s" % e)
            govobj.vote_delete(agnid)
            return (govobj, None)
    ...
    def vote_delete(self, agnid):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(agnid, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def sync_network_vote(self, agnid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = agnid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = AGNID_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 agni addr, non-multisig
            if not agnilib.is_valid_agni_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Agni address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (agnilib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AgniDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import agnilib
        obj_data = agnilib.SHIM_serialise_for_agnid(self.serialise())
    ...
    def prepare(self, agnid):
        try:
            object_hash = agnid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = AGNID_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not agnilib.is_valid_agni_address(addr, config.network):
    ...
    def hash(self):
        import agnilib
        return agnilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = AGNID_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, agnid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - agnid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, agnid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - agnid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, agnid):
        now = int(time.time())
        return (self.created_at < (now - agnid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, agnid):
        if self.is_expired(agnid):
            return False

        return True

    def is_deletable(self, agnid):
        if self.is_expired(agnid):
            return True
