...
def is_valid_agni_address(address, network='mainnet'):
    ...
    agni_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid Agni address
        return False

    if (address_version != agni_version):
        return False
...
# shims 'til we can fix the agnid side
def SHIM_serialise_for_agnid(sentinel_hex):
    from models import AGNID_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = AGNID_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in agnid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # agnid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    agnid_hex = serialise(obj)
    return agnid_hex


# shims 'til we can fix the agnid side
def SHIM_deserialise_from_agnid(agnid_hex):
    from models import AGNID_GOVOBJ_TYPES

    # unpack
    obj = deserialise(agnid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('agni.conf')
