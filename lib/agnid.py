...
class AgniDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some agnid methods
    ...
    @classmethod
    def from_agni_conf(self, agni_dot_conf):
        from agni_config import AgniConfig
        config_text = AgniConfig.slurp_config_file(agni_dot_conf)
        creds = AgniConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = agnilib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = agnilib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = agnilib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_agnid_proto_version_with_sentinel_ping)
